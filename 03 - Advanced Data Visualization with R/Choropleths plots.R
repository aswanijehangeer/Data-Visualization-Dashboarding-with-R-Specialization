...
# All we have to do to make a choropleth is to merge data into a data frame, 
# then we just use an aesthetic mapping like before, using fill.
...
ggplot(data = map_data_combined, 
       mapping = aes(x= long, y= lat, group = group, fill = qual_value))+
...
ggplot(data = map_data_combined, 
       mapping = aes(x= long, y= lat, group = group,fill=quant_value))+
  geom_polygon(color="black")
