---
title: "Week 1 Peer Review"
author: "Jehangeer"
date: "1/30/2021"
output: html_document
---

```{r setup, include = FALSE}
knitr::opts_chunk$set(echo = TRUE)

library(tidyverse)
data <- drop_na(read_csv("CELL DATA.csv"))

```
Your objective is to replicate these figures, created using the Center of Legislative Effectiveness Data. These figures are similar to those we completed in the lecture videos.

**IMPORTANT: Filter your data so you are only displaying information for the 115th Congress.**

## Exercise 1

Hints:

* For the y-axis, use the variable "all_pass".

* For the x-axis, use the variable "dwnom1".

* Make sure you recode the data for the "female" variable and rename it as "Gender" to generate the correct labels for the legend.

* Set the color aesthetic in the ggplot() function to make the color of the dots change based on Gender.

* Make sure the axis labels are correct.

```{r excercise 1, echo = FALSE, results = 'hide', fig.keep='last'} 
# filtering the data
data1 <- data %>% filter(congress == 115) %>% select("dwnom1", "all_pass")
head(data1)

# simple plot
ggplot(data1, aes(x = dwnom1, y = all_pass))+
      geom_point()

# adding labels
ggplot(data1, aes(x = dwnom1, y = all_pass))+
      geom_point()+
      labs(x = "ideology", y = "Bill Passed")

data1 <- data %>% 
      filter(congress == 115) %>%
      select("dwnom1", "all_pass", "female")

data1$female

ggplot(data1, aes(x = dwnom1, y = all_pass,color = female))+
      geom_point()+
      labs(x = "ideology", y = "Bill Passed")

# recoding female variable for colors

Gender <- recode(data1$female, '0' = "Male", '1' = "Female")
data1 <- add_column(data1,Gender)
head(data1)

ggplot(data1, aes(x = dwnom1, y = all_pass,color = Gender))+
      geom_point()+
      labs(x = "ideology", y = "Bill Passed")


```


## Exercise 2

Hints:

* For the x-axis, use the variable "votepct".

* For the y-axis, use "all_pass".

* Make sure you recode the data for the "female" variable to generate the correct labels for the legend. Rename that column "Gender". (you may have already done this in the last exercise)

* Make sure you recode the data for "majority" variable to generate the correct labels of the facetted figures.

* Set the color aesthetic in the ggplot() function to make the color of the dots change based on Gender.

* Use scale_color_manual() to set the colors to green for males and orange for females.

* Make sure the axis labels are correct.


```{r exercise2,echo=FALSE, results='hide', fig.keep='last'}

# filtering the data
data2 <- data %>%
      filter(congress == 115) %>%
      select("votepct", "all_pass", "majority")
head(data2)

ggplot(data2, aes(x = votepct, y = all_pass, color = Gender)) + 
      geom_point()+
      labs(x = "Vote Perentage", y = "Bill Passed")

facetlegends <- recode(data2$majority, '1' = "majority", '0' = "minority")
data2 <- add_column(data2, facetlegends)


ggplot(data2, aes(x = votepct, y = all_pass, color = Gender)) + 
      geom_point()+
      labs(x = "Vote Perentage", y = "Bill Passed")+ 
      scale_color_manual(values = c("orange", "green"))+
      facet_wrap(~facetlegends)


```


## Exercise 3

Hints:

* For the y-axis, use the variable "les".

* Make sure you recode the data for the "majority" variable to generate the correct labels. (you may have already done this in the last exercise)

* Make sure the axis labels and figure title are correct.


```{r exercise3,echo=FALSE, results='hide', fig.keep='last'}

# filtering the data
data3 <- data %>%
      filter(congress == 115) %>%
      select(x = "majority", y = "les")
data3

major <- recode(data3$x, '1' = "Majority", '0' = "Minority")
data3 <- add_column(data3,major)
data3

ggplot(data3, aes(x = major, y), color = major) + geom_boxplot()

# Adding labels and title

ggplot(data3, aes(x = major, y), color = major) +
      geom_boxplot() +
      labs(x = "Majority or Minority", y = "Legislative Effectiveness", title = "LES in 115th Congress")



```




